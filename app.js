...
function recordMatchesYear(r, y) {
  ...
  const nums = String(r.year || '').match(/20\d{2}/g) || [];
...
function convertInvestigationRow(row) {
  ...
  const archiveExact = row.archive_exact || '';
  const archiveHistory = row.archive_history || '';
  ...
  if (row.why_it_matters) {
    summaryParts.push(row.why_it_matters);
  }

  if (row.mime_type) {
    summaryParts.push(`Archived type: ${row.mime_type}`);
  }

  if (archiveExact) {
    summaryParts.push(`Exact archived capture available`);
  }

  if (archiveHistory) {
    summaryParts.push(`Wayback history available`);
  }

  const statusParts = [];

  if (row.priority) {
    statusParts.push(`${row.priority} priority`);
  } else {
    statusParts.push('Investigation');
  }

  if (row.score) {
    statusParts.push(`Score ${row.score}`);
  }

  if (row.captures) {
    statusParts.push(`${row.captures} captures`);
  }

  if (row.unique_versions) {
    statusParts.push(`${row.unique_versions} versions`);
  }
...
async function loadInvestigationRecords() {
  ...
  try {
    ...
    const investigationRecords = csvRows
      .map(convertInvestigationRow)
      .filter(r => r.title);

    const existing = new Set(
      records.map(
        r =>
          `${String(r.title || '').trim()}|${String(
            r.source || ''
          ).trim()}`
      )
    );
...
function renderYears() {
  ...
  yearGrid.innerHTML = YEARS.map(y => {
    const n = records.filter(r =>
      recordMatchesYear(r, y)
    ).length;

    return `
      <button
        type="button"
        class="year-btn ${y === yearFilter ? 'active' : ''}"
        data-year="${y}"
        aria-pressed="${y === yearFilter}"
      >
        <strong>${y}</strong>
        <span>${n} doc${n === 1 ? '' : 's'}</span>
      </button>
    `;
  }).join('');

  if (clearYear) {
    clearYear.hidden = yearFilter === 'All';
  }
}
...
function render() {
  ...
  const rows = records.filter(r => {
    const matchesYear =
      recordMatchesYear(r, yearFilter);
...
const tl =
  document.querySelector('#timelineList');
...
const sourceSearch =
  document.querySelector('#sourceSearch');

const sourceList =
  document.querySelector('#sourceList');

const sourceCount =
  document.querySelector('#sourceCount');
...
function renderSources() {
  ...
  sourceCount.textContent =
    `${rows.length} source${rows.length === 1 ? '' : 's'} indexed`;
...
async function startApp() {
  /*
    First render the records already
    contained in data.js.
  */
  ...
  /*
    Then load the investigation CSV.

    If loading succeeds, the new records
    are added to the existing explorer
  ...
  */
  ...
  /*
    Re-render after the CSV finishes
    loading.
  */
